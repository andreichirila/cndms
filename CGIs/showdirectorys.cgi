...
my $query_locations 		= $dbh->prepare("SELECT * FROM CameronetDokVer.Locations");
#my $query_documents 		= $dbh->prepare("SELECT * FROM CameronetDokVer.Dokumente");
my $query_documents_etiquet 	= $dbh->prepare("SELECT * FROM CameronetDokVer.DokumentBeschreibung");

$query_locations->execute() or die $query_locations->err_str;
#$query_documents->execute() or die $query_documents->err_str;
...
#while(	my ($d1,$d2,$d3,$d4,$d5,$d6,$d7,$d8,$d9,$d10,$d11) = $query_documents->fetchrow_array() ){
#	my %Daten = (	
#			"id"	 		=> $lastID += 1,
#			"doc_did" 		=> $d1,
#			"parent"	 	=> $d2,
...
while(my ($bid, $comments , $locationID, $blocked, $name, $type, $autor, $version, $date, $category) = $query_documents_etiquet->fetchrow_array()){
	my %Daten = (
		"id"		=>	$lastID += 1,
		"bid"		=>	$bid,
		"comments"	=>	$comments,
