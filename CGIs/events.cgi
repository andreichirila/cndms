...
my $reqChilds  	= $query->param("take_children_from");
my $version = $query->param("get_last_version");
...
if ($rename){
	...
	my $getVersion = $dbh->prepare("SELECT MAX(Version) FROM CameronetDokVer.Dokumente WHERE BID='$version'");
	$getVersion->execute();
	my $lastVersion = $getVersion->fetchrow_array();
	...
	my $children = $dbh->prepare("SELECT * FROM CameronetDokVer.DokumentBeschreibung WHERE LocationID='$reqChilds'") or die "Fehler bei der Datenabfrage\n\n";

	if( $children->execute() ){
		my $json        = "[";
		
		while(my ($bid, $comments , $locationID, $blocked, $name, $type, $autor, $version, $date, $category) = $children->fetchrow_array()){
		        my %Daten = (
		                "bid"           =>      $bid,
                		"comments"      =>      $comments,
		                "parent"        =>      $locationID,
...
#	$did 		 = $deleteArray->{DID};
	...
	if($bid and $bid >= 0){
		my $deleteInDB = $dbh->prepare("DELETE FROM CameronetDokVer.DokumentBeschreibung WHERE BID=$bid;");
